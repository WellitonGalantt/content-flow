...
export class AuthServices {
    static async createUser(data: IRegisterUser): Promise<Error | number> {
        try {
            return await db.transaction(async (trx) => { // Transaction serve para criar uma query em conjunto, para uma mesma transacao
                ...
                data.name = data.name.trim().replace(/\s+\g/, ' ')// rgex limpa espacos a mais que contem no nome: "Cleiyto   da Silva"
                    .toLowerCase()
                    .split(' ')
